...
const localCart = {
    ...
    updateQuantity: (productId, quantity) => { let cart = localCart.get(); const item = cart.find(p => p.id === productId); if(item) { if(quantity <= 0) cart = cart.filter(p => p.id !== productId); else item.quantity = quantity; } localCart.set(cart); return cart; },
...
async function signUp(email, password, userData) { 
    try { 
        ...
        if(!validatePassword(password)) throw new Error('Password must be 6+ chars');
        const {data, error} = await window.supabase.auth.signUp({
            email, password, 
            options: { data: {full_name: userData.full_name || '', phone: userData.phone || '', is_admin: false, created_at: new Date().toISOString()} }
        }); 
...
async function updateUserProfile(updates) { 
    try { 
        const {error} = await window.supabase.auth.updateUser({data: updates}); 
        if(error) throw error; 
        CacheManager.clear('user_profile'); 
        return true; 
    } catch(e){ throw e; } 
...
async function getProducts(category = null, limit = 100) { 
    try { 
        const cacheKey = `products_${category || 'all'}_${limit}`; 
        const cached = CacheManager.get(cacheKey); 
        if(cached) return cached;
        let query = window.supabase.from('products').select('*').limit(limit).order('created_at', {ascending: false});
...
async function editProduct(productId, updates) { 
    try { 
        const user = await checkUser();
        if(!user) throw new Error('Not authenticated');
        const profile = await getUserProfile();
        if(!profile.is_admin) throw new Error('Admin required');
        const {data, error} = await window.supabase.from('products').update(updates).eq('id', productId).select();
        if(error) throw error;
        CacheManager.clear('products');
        CacheManager.clear(`product_${productId}`);
        return data[0];
    } catch(e){ throw e; } 
}
...
async function addToCart(productId, quantity = 1) { 
    try { 
        ...
        localCart.add({id: productId, ...product, quantity});
        const {data, error} = await window.supabase.from('cart').insert([{
            user_id: user.id,
        ...
        CacheManager.clear(userCartKey);
        return data[0];
    } catch(e){ throw e; } 
}
...
async function getCart() { 
    try { 
        const user = await checkUser();
        if(!user) return localCart.get();
        const cacheKey = `cart_${user.id}`;
        const cached = CacheManager.get(cacheKey);
        if(cached) return cached;
        const {data, error} = await window.supabase.from('cart').select('*, products(*)').eq('user_id', user.id);
        if(error) throw error;
        CacheManager.set(cacheKey, data);
        return data || [];
    } catch(e){ return localCart.get(); }
}

async function updateCartQuantity(cartId, quantity) { 
    try { 
        if(quantity <= 0) { return await removeFromCart(cartId); }
        const {data, error} = await window.supabase.from('cart').update({quantity}).eq('id', cartId).select();
        if(error) throw error;
        const user = await checkUser();
        if(user) CacheManager.clear(`cart_${user.id}`);
        return data[0];
    } catch(e){ throw e; } 
}
...
async function removeFromCart(cartId) { 
    try { 
        const {error} = await window.supabase.from('cart').delete().eq('id', cartId);
        if(error) throw error;
        const user = await checkUser();
        if(user) CacheManager.clear(`cart_${user.id}`);
        return true;
...
async function clearCart() { 
    try { 
        const user = await checkUser();
        if(!user) return;
        const {error} = await window.supabase.from('cart').delete().eq('user_id', user.id);
        if(error) throw error;
        localCart.clear();
        CacheManager.clear(`cart_${user.id}`);
        return true;
    } catch(e){ throw e; } 
}
...
async function createOrder(orderData) { 
    try { 
        ...
        for(const item of cartItems) {
            ...
            if(!product) throw new Error(`Product not found`);
            if(product.stock < item.quantity) throw new Error(`Only ${product.stock} in stock`);
            const newStock = product.stock - item.quantity;
            await editProduct(item.product_id, {stock: newStock});
        }
        
        const orderNumber = `HEF-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
...
async function getOrders() { 
    try { 
        const user = await checkUser();
        if(!user) return [];
        const {data, error} = await window.supabase.from('orders').select('*').eq('user_id', user.id).order('created_at', {ascending: false});
        if(error) throw error;
        return data || [];
    } catch(e){ return []; } 
}
...
async function updateOrderStatus(orderId, status) { 
    try { 
        ...
        const {data, error} = await window.supabase.from('orders').update({status, updated_at: new Date().toISOString()}).eq('id', orderId).select();
        if(error) throw error;
        return data[0];
    } catch(e){ throw e; } 
}
...
async function addReview(productId, rating, comment = '') { 
    try { 
        ...
        if(!rating || rating < 1 || rating > 5) throw new Error('Rating 1-5 required');
        ...
        if(error) throw error;
        CacheManager.clear(`reviews_${productId}`);
        return data[0];
    } catch(e){ throw e; } 
}
...
async function getProductReviews(productId) { 
    try { 
        ...
        const cached = CacheManager.get(cacheKey);
        if(cached) return cached;
        const {data, error} = await window.supabase.from('reviews').select('*').eq('product_id', productId).order('created_at', {ascending: false});
        if(error) throw error;
        CacheManager.set(cacheKey, data);
        return data || [];
    } catch(e){ return []; } 
...
async function addToWishlist(productId) { 
    try { 
        const user = await checkUser();
        if(!user) throw new Error('Please login');
        const {data, error} = await window.supabase.from('wishlist').insert([{
            user_id: user.id,
            product_id: productId
        }]).select();
        if(error) throw error;
...
async function getWishlist() { 
    try { 
        ...
        const cached = CacheManager.get(cacheKey);
        if(cached) return cached;
        const {data, error} = await window.supabase.from('wishlist').select('*, products(*)').eq('user_id', user.id);
        if(error) throw error;
        CacheManager.set(cacheKey, data);
        return data || [];
    } catch(e){ return []; } 
...
async function removeFromWishlist(productId) { 
    try { 
        const user = await checkUser();
        if(!user) return;
        const {error} = await window.supabase.from('wishlist').delete().eq('user_id', user.id).eq('product_id', productId);
        if(error) throw error;
        CacheManager.clear('wishlist');
        return true;
    } catch(e){ throw e; } 
...
async function isItemInWishlist(productId) { 
    try { 
        ...
        const {data, error} = await window.supabase.from('wishlist').select('id').eq('user_id', user.id).eq('product_id', productId).single();
